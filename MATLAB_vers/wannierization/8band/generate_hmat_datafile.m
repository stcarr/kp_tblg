function [hmat_filename] = generate_hmat_datafile(filename, tar_folder, tar_theta)
    %filename = [tar_folder '/eightWANnew_H_' strrep(num2str(tar_theta,'%.2f'),'.','p') '_' num2str(numk) 'k.mat'];
    %wf_filename = [tar_folder '/eightWANnew_wf_' strrep(num2str(tar_theta,'%.2f'),'.','p') '_' num2str(numk) 'k.mat'];
    cutoff_scale = 4;

    load(filename);
    hmat_filename = [tar_folder '/hmat_8band_' strrep(num2str(tar_theta,'%.2f'),'.','p') '.dat'];

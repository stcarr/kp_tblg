function [h_filename, postproc_filename] = rewannier_aapm(filename,tar_folder,tar_theta,numk,k_radius,filetype)

    %clearvars;
    % Re-Wannierization of the effective TB

    h_filename = [tar_folder '/hmat_8band_' strrep(num2str(tar_theta,'%.2f'),'.','p') '_rewan.dat'];
    ...
    PDOS_sum = zeros(4,num_orbs);
    PDOS_sum(1,1:2)=[1,1];  % AA_pm
    PDOS_sum(2,3)=1;        % AA_z
    PDOS_sum(3,4:5)=[1,1];  % AB/BA
    PDOS_sum(4,6:8)=[1,1,1];% DW
    ...
    for krun = 1:NkGridPts
        ...
        % PDOS-original
        PDOS((krun-1)*num_orbs + (1:8),1,1) = E0;
        PDOS((krun-1)*num_orbs + (1:8),2:5,1) = (PDOS_sum*power(abs(Psi_orig),2)).'/NkGridPts;
        ...
        % PDOS-new
        PDOS((krun-1)*num_orbs + (1:8),1,2) = E0;
        PDOS((krun-1)*num_orbs + (1:8),2:5,2) =...
    ...
    PDOS_plot0 = zeros(Nbins,2,4);
    PDOS_plot1 = zeros(Nbins,2,4);
    for channel = 1:4

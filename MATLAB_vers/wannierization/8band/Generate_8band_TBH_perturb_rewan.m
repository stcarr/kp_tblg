function [H_perturb, hex_R, grid_to_index] = Generate_8band_TBH_sym_ver4_rewan(filename, rewan_filename)
    ...
    grid_to_index = zeros(2*MMM+1);
    
    clear hex_R;
    indh=1;
    for ind1=(-MMM):MMM
        for ind2=(-MMM):MMM
            vectmp=ind1*(moire_L_x1-moire_L_x2)+ind2*moire_L_x1;


            if norm(vectmp)<=hop_cut
                hex_R(indh,:)=vectmp(:);
                grid_to_index(ind1+MMM+1,ind2+MMM+1) = indh;
                indh=indh+1;
    ...
    for indr=1:num_hex
        ...
        for indk=1:knum_tot
            ...
            U_now = squeeze(U_store(:,:,indk));
            Heff_now=U_now'*squeeze(h_eff_perturb(1:8,1:8,indk))*U_now;

            all_R_mat(:,:,indr)=all_R_mat(:,:,indr)+Heff_now*exp(i*dot(know,R_hop))/(numk^2);
    ...
    % Start of "band structure" comment block
    %{
    ...
    for indk=1:knum_tot
        ...
        for indr=1:num_hex
            ...
            for indks=1:6
                ...
                allHmat(:,:,indks)=allHmat(:,:,indks)+squeeze(all_R_mat_cut(:,:,indr))*exp(-i*dot(know,rvec_now));
    ...
    % End of "band structure" comment block
    %}

function [filename] = fiveWANcs(tar_theta,tar_folder,numk,numxy,side)
    ...
    filename = [tar_folder '/fiveWANcs' side_text '_data_' strrep(num2str(tar_theta,'%.2f'),'.','p') '_' num2str(numk) 'k_' num2str(numxy) 'xy.mat'];
    ...
    all_wfAL1(:,:,3)=wfAL1(:,:);
    all_wfBL1(:,:,3)=wfBL1(:,:);
    all_wfAL2(:,:,3)=-sym_sign*wfAL2(:,:);
    all_wfBL2(:,:,3)=-sym_sign*wfBL2(:,:);
    ...
    all_ssdiag=zeros(5,knum_tot);

    allbands=zeros(tot_dim,knum_tot);

    allbands_eff=zeros(5,knum_tot);

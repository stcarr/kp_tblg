function [filename] = eightWANcs(tar_theta,tar_folder,numk,numxy)

    filename = [tar_folder '/eightWANcs_data_' strrep(num2str(tar_theta,'%.2f'),'.','p') '_' num2str(numk) 'k_' num2str(numxy) 'xy.mat'];
    ...
    %numxy=51;%501; passed to function
    allxy=2.0*LLmoire*linspace(-1,1,numxy);

    midpt=(numxy+1)/2;
    [mmX,mmY]=meshgrid(allxy,allxy);

    all_wfAL1=zeros(numxy,numxy,5);
    all_wfBL1=zeros(numxy,numxy,5);
    all_wfAL2=zeros(numxy,numxy,5);
    all_wfBL2=zeros(numxy,numxy,5);

    % 1: B dimer on AA (AA_+)
    wfAL1=0*mmX;
    wfAL2=0*mmX;
    wfBL1=exp(-(mmX.^2+mmY.^2)/2/13^2);
    wfBL2=-i*exp(-(mmX.^2+mmY.^2)/2/13^2);

    % wfAL1=wfAL1/sqrt(sum(abs(wfAL1(:)).^2));
    wfBL1=wfBL1/sqrt(sum(abs(wfBL1(:)).^2));
    % wfAL2=wfAL2/sqrt(sum(abs(wfAL2(:)).^2));
    wfBL2=wfBL2/sqrt(sum(abs(wfBL2(:)).^2));
    ...
    % 2: A dimer on AA (AA_-)
    wfAL1=exp(-(mmX.^2+mmY.^2)/2/13^2);
    wfAL2=i*exp(-(mmX.^2+mmY.^2)/2/13^2);
    wfBL1=0*mmX;
    wfBL2=0*mmX;
    ...
    % clear wavefunction variables
    wfAL1=0*mmX;
    wfAL2=0*mmX;
    wfBL1=0*mmX;
    wfBL2=0*mmX;
    
    % 3: AA p_z style orbital (AA_z)

    % L1A : (x-iy) phase
    tmp=(mmX-i*mmY);
    tmp(tmp~=0)=tmp(tmp~=0)./abs(tmp(tmp~=0));
    tmp(midpt,midpt)=0;

    % L1B : (-x-iy) phase
    tmpb=-mmX-i*mmY;
    tmpb(tmpb~=0)=tmpb(tmpb~=0)./abs(tmpb(tmpb~=0));
    tmpb(midpt,midpt)=0;

    % L2B: (-x-iy) phase
    tmp2=(-mmX-i*mmY);
    tmp2(tmp2~=0)=tmp2(tmp2~=0)./abs(tmp2(tmp2~=0));
    tmp2(midpt,midpt)=0;

    % L2A: (x-iy) phase
    tmp2b=mmX-i*mmY;
    tmp2b(tmp2b~=0)=tmp2b(tmp2b~=0)./abs(tmp2b(tmp2b~=0));
    tmp2b(midpt,midpt)=0;

    % don't center gaussian on AA stacking, wannier has zero weight there.
    wf_setpt=20;

    % 
    wfAL1=tmp.*exp(-(sqrt(mmX.^2+mmY.^2)-wf_setpt).^2/2/9^2);
    wfAL1=wfAL1/sqrt(sum(abs(wfAL1(:)).^2));

    wfBL1=tmpb.*exp(-(sqrt(mmX.^2+mmY.^2)-wf_setpt).^2/2/9^2);
    wfBL1=wfBL1/sqrt(sum(abs(wfBL1(:)).^2));

    wfBL2=tmp2.*exp(-(sqrt(mmX.^2+mmY.^2)-wf_setpt).^2/2/9^2);
    wfBL2=wfBL2/sqrt(sum(abs(wfBL2(:)).^2));

    wfAL2=tmp2b.*exp(-(sqrt(mmX.^2+mmY.^2)-wf_setpt).^2/2/9^2);
    wfAL2=wfAL2/sqrt(sum(abs(wfAL2(:)).^2));
    ...
    % 4: AB orbital
    honeycomb_center=(moire_L_x1+moire_L_x2)/3;
    ...
    wfBL2=0*mmX;
    sigmaxy=30;
    tmpwf=exp(-((mmX-honeycomb_center(1)).^2+(mmY-honeycomb_center(2)).^2)/2/(sigmaxy^2));
    tmpwf=tmpwf/sqrt(sum(abs(tmpwf(:)).^2));

    % p_z style: +- between L1 and L2
    wfAL1=tmpwf;
    wfBL2=-tmpwf;
    ...
    % checks something about the honeycomb center (in the right place relative to moire pattern)?
    cc1=TT1_0*exp(i*dot(honeycomb_center,KTH*[1,0,0]));
    cc2=TT2_0*exp(i*dot(honeycomb_center,KTH*[-0.5,-0.5*sqrt(3),0]));
    cc3=TT3_0*exp(i*dot(honeycomb_center,KTH*[-0.5,0.5*sqrt(3),0]));
    ...
    % mirror
    wfBL2=kag_sign*tmpwf*shift_L2B;

    % C2z T
    wfBL1=tmpwf*shift_L1B;

    % mirror from above
    wfAL2=kag_sign*tmpwf*shift_L2A;
    ...
    knum=numk; % used to be knum = 9, now passed to function
    ...
    sel_bandind=(fff-3):(fff+4); % 8 bands!
    ...
    all_ssdiag=zeros(8,knum_tot);

    allbands=zeros(tot_dim,knum_tot);

    allbands_eff=zeros(8,knum_tot);
    ...
    scatter3(all_kpts(:,1),all_kpts(:,2),squeeze(all_ssdiag(5,:)));
